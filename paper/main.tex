...
%
% INFRA IS DONE. You do not need to set up LaTeX.
%   - Overleaf: New Project -> Upload, drop this file + the paper/figures/*.png,
%     set the compiler to pdfLaTeX. The llncs class is built into Overleaf.
%   - Sections marked  % >>> TASK: ...  are the pickup-able pieces (see paper/TASKS.md).
...
% >>> TASK (Edward): finalize author list + affiliations before submission.
...
%     ~4--6 sentences. See paper/TASKS.md "Intro hook".
...
\textbf{[RELATED WORK — Saanvi \& Alex. See docs/related\_works.md.]}

% \subsection{EEG-based Parkinson's detection.}        % Saanvi
% \subsection{Evaluation pitfalls and site effects in EEG-DL.}  % Saanvi
% \subsection{Self-supervised learning for EEG.}       % Alex
% \subsection{Calibration under domain shift.}         % Alex
